...
<script lang="ts">
	...
	import ActivityPanel from '$lib/play/admin/activity_panel.svelte';
	...
	let deliberation_active = $state(false);
	...
	socket.on('set_question_number', (data) => {
		...
		deliberation_active = data.question?.interaction_mode === PEER_DELIBERATION;

		clearInterval(timer_interval);
		if (deliberation_active) {
			game_state.timer_res = '0';
			return;
		}
...
{#if deliberation_active}
	<ActivityPanel
		question_index={game_state.shown_question_now}
